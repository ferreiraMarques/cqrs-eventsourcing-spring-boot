...
@RestController
@RequestMapping(path = "/api/v1/closedAccount")
public class ClosedAccountController {
	...
	@PutMapping(path = "/{id}")
	public ResponseEntity<BaseResponse> closedAccount(@PathVariable("id") String id) {
		try {
			dispacther.send(new CloseAccountCommand(id));
			return new ResponseEntity<>(new BaseResponse("Correct withdraw into account"), HttpStatus.OK);
		} catch (IllegalStateException | AggregateNotFoundException e) {
			logger.log(Level.WARNING,
					MessageFormat.format("Error closed account {0}", e.getMessage().toString()));
